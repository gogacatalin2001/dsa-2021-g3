...
int searchElem(ListT *listpt, int element)
{
    NodeT *p;
    p=listpt->first;
    while(p!=NULL)
...
int deleteByKey (ListT *listpt,int key)
{
    NodeT *q=listpt->first,*q1=NULL;
    while(q!=NULL)
    {
    ...
    if(q!=NULL)
    {
        if(q==listpt->first)
        {
            listpt->first=listpt->first->next;
            listpt->nr--;
           if(listpt->nr==1)
	{
	listpt->last=listpt->first;
	listpt->last->next=NULL;
    }
            if(listpt->first==NULL)
                listpt->last=NULL;
        }
        else
        {
            q1->next=q->next;
            if(q==listpt->last)listpt->last=q1;
            listpt->nr--;
        }
        return 1;
    }
...
//we dont use DeleteTheFirst as in the fist problem, we use only deleteByKey
void X(int truck_id,ListT *listPtrRoad,ListT *listPtrGarage,FILE *fo)
{
    if(listPtrGarage->first->truck_id==truck_id)
    {
        ...
        deleteByKey(listPtrGarage,truck_id);
    }
